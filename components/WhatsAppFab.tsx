...
export default function WhatsAppFab() {
  const [isPriceSectionVisible, setIsPriceSectionVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    const observePriceSection = () => {
      observer?.disconnect();
      if (!mobileQuery.matches) {
        setIsPriceSectionVisible(false);
        return;
      }

      observer = new IntersectionObserver(
        ([entry]) => setIsPriceSectionVisible(entry.isIntersecting),
        { threshold: 0.08 },
      );
    ...
    observePriceSection();
    mobileQuery.addEventListener("change", observePriceSection);
    return () => {
      observer?.disconnect();
      mobileQuery.removeEventListener("change", observePriceSection);
    };
  }, []);

  return (
    <a
      className={`whatsapp-fab${isPriceSectionVisible ? " whatsapp-fab--hidden-on-price" : ""}`}
      href="https://wa.me/4917646687719?text=Hallo%2C%20ich%20interessiere%20mich%20f%C3%BCr%20Ihre%20Alltagshilfe."
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Helfer im Alltag über WhatsApp kontaktieren"
      aria-hidden={isPriceSectionVisible || undefined}
      tabIndex={isPriceSectionVisible ? -1 : undefined}
    >
